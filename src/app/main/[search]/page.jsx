...
const Main = ({ params }) => {
  ...
  function downloadAudio(id,digit){
    const url = [`https://www.yt2mp3s.me/api/widget?url=https://www.youtube.com/watch?v=${id}`,
  ...
  return (
    ...
    <div class="overlayItem w-4/5 h-3/4 bg-white px-1">
      ...
      {dataG ?
        dataG.map((element, index) => (
          <div key={index} className='card bg-newGrey flex flex-col justify-between p-4'>
            <div>
              <div className=''>
                <img src={element.thumbnails[1].url} alt="Image Preview Unavailable" className='rounded-md w-full text-white'/>
              </div>
              <div>
